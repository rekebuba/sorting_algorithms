...
void check_word(deck_node_t **deck, deck_node_t *curent, deck_node_t *next)
{
    ...
    {
        if (words[curent->card->value[0]] > words[(next->card->value[0])] && curent->card->kind >= next->card->kind)
        {
    ...
    {
        if (words[curent->card->value[0]] > value_2 && curent->card->kind >= next->card->kind)
        {
            swap_deck(deck, curent, next);
            print_deck(*deck);
        }
    ...
    {
        if (words[(next->card->value[0])] > value_1 && curent->card->kind >= next->card->kind)
        {
            swap_deck(deck, curent, next);
            print_deck(*deck);
        }
